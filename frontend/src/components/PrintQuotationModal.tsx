          ...
          >
            ...
            <div className="grid grid-cols-2 border-b border-black print-avoid-break">
              <div className="p-2 border-r border-black space-y-0.5 text-[10.5px]">
                ...
                {cfg.showCustomerName && <p className="font-bold text-black text-xs uppercase">{party.name}</p>}
                ...
                {cfg.showGstin && party.gstin && <p className="font-bold text-black">GSTIN : {party.gstin}</p>}
              </div>
            ...
            <table className="w-full text-[10.5px] border-b border-black border-collapse">
              <thead>
                <tr className="bg-gray-100 font-bold border-b border-black text-black">
                  ...
                  {cfg.showItemName && <th className="p-1 border-r border-black text-left">Item name</th>}
                  {cfg.showHsn && <th className="p-1 border-r border-black text-center w-24">HSN/ SAC</th>}
                  {cfg.showQuantity && <th className="p-1 border-r border-black text-center w-16">Quantity</th>}
                  {cfg.showRate && <th className="p-1 border-r border-black text-right w-24">Price/ Unit</th>}
                  {(cfg.showGstRate || cfg.showTaxAmount) && <th className="p-1 border-r border-black text-right w-28">GST</th>}
                  {cfg.showTotalAmount && <th className="p-1 text-right w-24">Amount</th>}
                </tr>
              </thead>
              <tbody>
                {items.map((item: any, idx: number) => {
                  ...
                  return (
                    <tr key={item.id || idx} className="border-b border-gray-400 print-avoid-break">
                      ...
                      {cfg.showHsn && <td className="p-1.5 border-r border-black text-center font-mono">{item.hsnSac}</td>}
                      {cfg.showQuantity && <td className="p-1.5 border-r border-black text-center font-bold">{item.quantity}</td>}
                      {cfg.showRate && (
                        <td className="p-1.5 border-r border-black text-right font-mono">
                          ₹ {item.rate.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                        </td>
                      )}
                      {(cfg.showGstRate || cfg.showTaxAmount) && (
              ...
              <tfoot>
                <tr className="bg-gray-100 font-bold border-t border-black text-[10.5px] print-avoid-break">
                  <td colSpan={cfg.showItemName ? 2 : 1} className="p-1.5 border-r border-black text-left font-bold">Total</td>
                  {cfg.showHsn && <td className="p-1.5 border-r border-black text-center"></td>}
                  {cfg.showQuantity && <td className="p-1.5 border-r border-black text-center font-bold">{totalQty}</td>}
                  {cfg.showRate && <td className="p-1.5 border-r border-black"></td>}
                  {(cfg.showGstRate || cfg.showTaxAmount) && (
                  ...
                  <>
                    {cfg.showCgst && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">₹ {totalCgstSum.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                      </div>
                    )}
                    {cfg.showSgst && (
                      <div className="flex justify-between">
                        <span>SGST Total:</span>
                        <span className="font-mono">₹ {totalSgstSum.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                      </div>
                    )}
                  </>
                ) : (
                  <>
                    {cfg.showIgst && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">₹ {totalIgstSum.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                      </div>
                    )}
                  </>
                )}
            ...
            {/* Bottom Footer: Bank Details | Terms Snapshot | Signature */}
