...
Smart Agro Machinerys`;
  ...
  return (
    <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex flex-col items-center justify-center p-2 sm:p-4 overflow-y-auto font-sans">
      ...
      {/* Top Action Bar */}
      <div className="bg-slate-900 text-white rounded-t-2xl max-w-4xl w-full px-6 py-3 flex items-center justify-between shadow-xl print:hidden border-b border-slate-800">
        ...
        >
          ...
          <div className="flex justify-between items-start border-b-2 border-slate-900 pb-4 mb-4">
            ...
            <div className="text-right space-y-1">
              ...
              <div className="font-mono text-sm font-extrabold text-slate-900 mt-1">{challan.challanNumber}</div>
              <div className="font-mono text-xs font-bold text-slate-700">Date: {new Date(challan.challanDate).toLocaleDateString('en-IN')}</div>
              {challan.refOrderNo && <div className="font-mono text-[11px] text-slate-600">Ref / Invoice No: {challan.refOrderNo}</div>}
              {challan.poNumber && <div className="font-mono text-[11px] text-slate-600">PO No: {challan.poNumber}</div>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4 text-[11px]">
            ...
            <div className="border border-slate-300 rounded-xl p-3 bg-slate-50/50 space-y-1">
              ...
              <div className="font-extrabold text-sm text-slate-900">{party.name || 'N/A'}</div>
              ...
              {party.gstin && <div className="font-mono font-bold text-slate-900">GSTIN: {party.gstin}</div>}
            ...
            <div className="border border-slate-300 rounded-xl p-3 bg-amber-50/40 space-y-1">
              ...
              {challan.vehicleNumber && <div className="font-mono font-bold text-slate-900">Vehicle No: {challan.vehicleNumber}</div>}
              {challan.transporter && <div className="font-semibold text-slate-800">Transporter: {challan.transporter}</div>}
            </div>
          </div>
            ...
            <table className="w-full text-left text-[11px] border-collapse">
              <thead className="bg-slate-900 text-white font-extrabold text-[10px] uppercase">
                <tr>
                  ...
                  <th className="p-2">Item / Equipment Description</th>
                  ...
                  <th className="p-2 w-16 text-center">Free Qty</th>
                  <th className="p-2 w-28 text-center">Serial Number</th>
                </tr>
              </thead>
                ...
                {items.map((item: any, index: number) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'}>
                    ...
                    <td className="p-2">
                      ...
                      {item.description && <div className="text-[10px] text-slate-600 leading-tight">{item.description}</div>}
                      {item.notes && <div className="text-[10px] text-amber-800 font-semibold">{item.notes}</div>}
                    </td>
                    <td className="p-2 text-center font-mono font-semibold text-slate-700">{item.hsnSac || '-'}</td>
                    <td className="p-2 text-center font-semibold text-slate-700">{item.unit || 'Nos'}</td>
                    <td className="p-2 text-center font-black font-mono text-xs">{item.quantity}</td>
                    <td className="p-2 text-center font-bold font-mono text-slate-600">{item.freeQuantity || 0}</td>
                    <td className="p-2 text-center font-mono font-extrabold text-emerald-800">{item.serialNumber || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4 text-[11px] bg-slate-50 p-3 rounded-xl border border-slate-200">
            ...
            <div className="text-right space-y-0.5 font-medium">
              <div>Transporter / Vehicle: <span className="font-bold text-slate-900">{challan.transporter || 'Direct Delivery'}</span></div>
              {challan.vehicleNumber && <div>Vehicle Reg No: <span className="font-mono font-bold text-slate-900">{challan.vehicleNumber}</span></div>}
